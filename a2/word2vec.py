...
def naiveSoftmaxLossAndGradient(
    ...
    # to integer overflow.
    # centerWordVec = centerWordVec.reshape((-1,1))   # (word vector length, 1) 不加的话维度不对 其实这里理解错了，centerWordVec是一个行向量，当成列向量了，详见向量和矩阵的维度表示区别
    # sim = np.matmul(outsideVectors, centerWordVec)  # (num words in vocab, 1)
    # softVector = softmax(sim.reshape((1,-1))).reshape((-1,1))  # (num words in vocab, 1)
    # loss = -np.log(softVector[outsideWordIdx])  # a number
    # softCopy = softVector.copy()    # (num words in vocab, 1)
    # softCopy[outsideWordIdx] -= 1   # (num words in vocab, 1)
    # gradCenterVec = np.matmul(outsideVectors.T, softCopy)   # (word vector length, 1)
    # gradOutsideVecs = np.matmul(softCopy, centerWordVec.T)   # (num words in vocab, word vector length)
    sim = np.dot(outsideVectors, centerWordVec)
    softVector = softmax(sim)
    loss = -np.log(softVector[outsideWordIdx])
    softCopy = softVector.copy()
    softCopy[outsideWordIdx] -= 1
    gradCenterVec = np.dot(outsideVectors.T, softCopy).flatten()
    gradOutsideVecs = np.dot(softCopy.reshape((-1,1)), centerWordVec.reshape((1,-1)))
    # gradOutsideVecs=np.outer(softCopy,centerWordVec)
    # END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    # # f.write('init of loss is:'+str(-np.log(sigmoid(logit_pos))))
    # # f.write("-"*20)
    ...
    tmp = sigmoid(np.dot(uo, centerWordVec))
    loss = -np.log(tmp)
    # f.write('init of loss is:'+str(loss))
    # f.write("-"*20)
    gradCenterVec = (tmp-1)*uo

    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = (tmp-1)*centerWordVec
    for indice in indices[1::]:
        uk = outsideVectors[indice]
        tmp = sigmoid(np.dot(uk, centerWordVec))
        loss -= np.log(1-tmp)
        gradCenterVec += tmp*uk.flatten()
        gradOutsideVecs[indice] += tmp*centerWordVec
    # f.write(str(loss))
    # f.write("="*20)
    # f.close()
    return loss, gradCenterVec, gradOutsideVecs
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centerWordVecIdx = word2Ind[currentCenterWord]
    centerWordVec = centerWordVectors[centerWordVecIdx]
    for word in outsideWords:   # 一个loss对应一个loss，固定vc的grad，所有出现的vo的grad，这里是求和，上述三个元素值都应该累加
        outsideWordIdx = word2Ind[word]
        tmp_loss, tmp_gradCenter, tmp_gradOutside = word2vecLossAndGradient(
            centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += tmp_loss
        # print(gradCenterVecs[centerWordVecIdx].shape,tmp_gradCenter.shape)
        gradCenterVecs[centerWordVecIdx] += tmp_gradCenter
        gradOutsideVectors += tmp_gradOutside
    # print(loss)
    # END YOUR CODE
