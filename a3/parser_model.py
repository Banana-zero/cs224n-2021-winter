...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings, 0, w.flatten()).reshape((-1, self.n_features*self.embed_size))
    ...
    def forward(self, w):
        ...
        w_emb=self.embedding_lookup(w)
        hidden=F.relu(torch.mm(w_emb,self.embed_to_hidden_weight)+self.embed_to_hidden_bias    )
        # (batch_size,n_features*embed_size)*(embed_size*n_features,hidden_size)=(batch_size,hidden_size)
        if self.training:
